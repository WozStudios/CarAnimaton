...
void DecalGenerator::Generate()
{
	...
	_decals.push_back(new Decal(transform, 12, -90.0f, _cameraPosition, _cameraDirection));

	// Cracks
	transform.position = vec3(50.0f, 45.0f, -90.0f);
	transform.rotation = vec3(0.0f, 1.0f, 0.0f);
	transform.scale = vec3(5.0f, 5.0f, 1.0f);
	_decals.push_back(new Decal(transform, 8, -90.0f, _cameraPosition, _cameraDirection));
}